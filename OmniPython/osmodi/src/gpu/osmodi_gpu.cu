...
// isnan2 helper function for GPU
template <typename varfloat>
...
bool InitializeGPU(SolverParameters<varfloat> SP) {
    // Detects GPUs and selects the one with most memory
    if (SP.Verbose) { 
    ...
    if (NumberOfGPUs == 0) {
        cout << "\n***Warning! No GPU found in the system!***" << endl;
        return false;
    }
    ...
    for (int i = 0; i < NumberOfGPUs; i++) {
        ...
        if (SP.Verbose) {
            ...
            //cout << "  Memory Clock Rate (MHz): " << prop.memoryClockRate / 1.0e3 << endl;
           // cout << "  Memory Clock Rate (KHz): " << prop.memoryClockKHz << endl;
            cout << "  Device Global Memory (GB): " << ThisDeviceMem / 1.0e9 << endl << endl;
        }
    }

    if (LargestDevID >= 0) {
        if (SP.Verbose) {
            cout << "\nUsing Device: " << LargestDevName << endl;
        }
        cudaSetDevice(LargestDevID);
        return true;
    }
    else {
        cout << "\n***Warning! Found GPU device but for some reason code failed!***" << endl;
        return false;
    }
}
...
void ConjugateGradientSolver_GPU(varfloat* PressureField, varfloat* RHS, SolverParameters<varfloat> SolverConfig, BoxContents<varfloat> VTK_Contents) {
    ...
    while (!canExit){
        ...
        vectorDot_GPU<<<numBlocks1D, threadsPerBlock1D, 0, stream[1]>>>(d_rk, d_rk, d_r_norm_old, d_SolverConfig); //r_k dot r_k
        cudaDeviceSynchronize();
        ...
        CGS_Progress.clear();
        P_cgs.Iteration = 0; P_cgs.Residual = 1.0f; P_cgs.TimeSeconds = 0.0; 
        ...
        for (int cgs_iter = 0; cgs_iter < VTK_Contents.totalBoxElements; cgs_iter++) {
            ...
            varfloat r_norm; 
            cudaMemcpy(&r_norm, d_r_norm, sizeof(varfloat), cudaMemcpyDeviceToHost);
            cudaDeviceSynchronize();
            r_norm = sqrt(r_norm);
            ...
            if (cgs_iter % 10 == 0) {
                if (SolverConfig.Verbose){ 
                    cout << "CG Iteration=" << cgs_iter 
                         << "; RelRes=" << scientific << r_norm / r_norm_init 
                         << "; AbsRes=" << r_norm << endl;
                }
            }

            //Stores iteration info on memory
            P_cgs.Iteration = cgs_iter+1; P_cgs.Residual = r_norm / r_norm_init; P_cgs.TimeSeconds = 0.0; 
            CGS_Progress.push_back(P_cgs);
            ...
            if ((r_norm / r_norm_init > SolverConfig.solverToleranceRel) && (r_norm > SolverConfig.solverToleranceAbs)) {
                ...
                if (SolverConfig.Verbose){ 
                    cout << "CG Iteration=" << cgs_iter 
                         << "; RelRes=" << scientific << r_norm / r_norm_init 
                         << "; AbsRes=" << r_norm << " [Converged]" << endl;
                }

                if (isnan(r_norm)) {
                    cout << "======== Result was NAN! ========" << endl;
                    cout << "Make sure your coordinate system is correct. This code expects a coordinate in the 'ND grid' format, i.e., dimension order is (x, y) or (x, y, z). DO NOT USE the 'MESHGRID' format to build the arrays!" << endl;
                }
                      ...
                      py::dict options) 
{
    ...
    if (!options.is_none()) {
        ...
        if (options.contains("SolverDevice")) {
            ...
            if (iequals(device, "CPU")) {
                ...
                if (SolverConfig.Verbose) cout << "SolverConfig.SolverDevice = GPU " << endl;
            } else {
    ...
    if (SolverConfig.SolverDevice == GPU) {
        GPUSuccess = InitializeGPU(SolverConfig);
        if (!GPUSuccess) {
            cout << "***GPU initialization failed. Defaulting to CPU.***" << endl;
        }
    }
    if ((SolverConfig.SolverDevice == GPU) && GPUSuccess) {
        if (SolverConfig.Verbose){
            cout << "========Starting GPU Solver========" << endl;
        }
...
// PyBind11 Module 
PYBIND11_MODULE(osmodi_gpu_bind, m) {
    m.doc() = "OSMODI solver interface (CPU+GPU, float/double, 2D/3D)";
